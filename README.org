...
- *Basic API:* Buffer name generation, mode mapping
...
- *Code Quality:* Frame capture race condition fix, timer cleanup in hide, vterm-setup docstring
...
All tests should pass. The exact count will change as coverage grows. A current
run looks like:
